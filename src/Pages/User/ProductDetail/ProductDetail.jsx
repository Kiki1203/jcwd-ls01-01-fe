...
function ProductDetail(props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      await axios.get(`${API_URL}/product/productdetail?id=${productId}`, {headers: {'Access-Control-Allow-Origin': '*'}})
      .then(res => {
        setProduct({...res.data})
      })
      .catch(e => {
...
useEffect(() => {
  ...
  if(Object.keys(product).length !== 0){
    axios.get(`${API_URL}/product/relatedproducts?id=${productId}&keluhanid=${product.keluhanId}&golonganobatid=${product.golonganObatId}`,
            ...
            <div className='d-flex'>
                ...
                <div id='right-side'>
                  ...
                  </div>
                  {
                    ...
                    ? <div style={{marginTop:'20px'}}><span id='butuh-resep'>Pembelian produk ini membutuhkan resep dokter.</span></div>
                    : <div className='d-flex align-items-center mt-2'>
                    <button id='qty-button-left' disabled={qty === 1} onClick={() => setQty(qty - 1)}><FontAwesomeIcon icon={faMinus} className='qty-icon' /></button>
                    <input type="number" value={qty} id='qty-number'
                    onChange={(e) => {
                      if(e.target.value.length !== 0){setQty(e.target.valueAsNumber)}
                      }} />
                    <button id='qty-button-right' disabled={qty >= product.stok} onClick={() => setQty(qty + 1)}><FontAwesomeIcon icon={faPlus} className='qty-icon' /></button>
                  ...
                  <div className='d-flex' style={{marginTop:'40px'}}>
                    {
                      product.butuhResep === 'Ya'
                      ? <button id='button-beli' style={{marginLeft:'0px'}}>Upload Resep</button>
                      : <>
                        ...
                        <button id='button-beli' disabled={qty > product.stok}>Beli</button>
                      </>
                    }
                    <button id='button-favorit'><FontAwesomeIcon icon={faHeart} /></button>
                  </div>
                  <div id='box-deskripsi'>
                    <div className='d-flex'>
                      <div className={'header-tab ' + (activeTab === 'deskripsi' && 'header-tab-highlight')}
                      onClick={() => setActiveTab('deskripsi')}>Deskripsi</div>
                      <div className={'header-tab ' + (activeTab === 'caraPakai' && 'header-tab-highlight')}
                      onClick={() => setActiveTab('caraPakai')}>Cara Pakai</div>
                      <div className={'header-tab ' + (activeTab === 'peringatan' && 'header-tab-highlight')}
                      onClick={() => setActiveTab('peringatan')}>Peringatan</div>
                    </div>
                    ...
                    <div className={'deskripsi-paragraf ' + (activeTab === 'caraPakai' && 'd-block')}>
                      {product.caraPakai}
                    </div>
                    <div className={'deskripsi-paragraf ' + (activeTab === 'peringatan' && 'd-block')}>
                      {product.peringatan}
                    </div>
                  </div>
                </div>
